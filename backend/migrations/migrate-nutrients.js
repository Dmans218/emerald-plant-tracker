    ...
    for (const [brandKey, brandData] of Object.entries(nutrientBrands)) {
      ...
      for (const [stage, stageProducts] of Object.entries(products)) {
        if (Array.isArray(stageProducts)) {
          for (let i = 0; i < stageProducts.length; i++) {
            const product = stageProducts[i];

            // Determine mixing order - products are already in correct order in data file
            let mixingOrder = 999; // default for supplements
            if (stage !== 'supplements') {
              mixingOrder = i + 1; // Use array position as mixing order for base nutrients
            }

            await client.query(
              `INSERT INTO nutrient_products
               (brand_id, name, growth_stage, ratio, unit, is_optional, is_flowering_only, is_hydro_only, is_early_growth, week_range, mixing_order, created_at)
